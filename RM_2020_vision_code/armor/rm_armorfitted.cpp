 ...
 * @param roi ROI对象
 */
void CandidateArmor::inputParam(NiceLight &light_left, NiceLight &light_right,
                                RM_SolveAngle &angle_solve, ArmorROI &roi) {
  ...
  // float _w = float(sqrt(pow(fabs(light_left.depth - light_right.depth), 2) +
  // pow(armor_width, 2)));
  // RotatedRect _rect =
  // RotatedRect(rect.center,Size(_w,rect.size.height),rect.angle);
 ...
 * @return RotatedRect
 */
RotatedRect CandidateArmor::fit_Rrect(RotatedRect &rect_left,
  ...
  // float center_slope =
  // (rect_left.center.y-rect_right.center.y)/(rect_left.center.x-rect_right.center.x);
  ...
  float H = (height_left + height_right) / 2;  //*2.27;
 ...
 *
 * @note 如果 IS_NUMBER_PREDICT_OPEN
 * 宏定义使能，则会导入tensorflow识别手写数字的模型
...
RM_ArmorFitted::RM_ArmorFitted() {
  ...
  /*------------读取模型------------*/
  status_load = tensorflow::ReadBinaryProto(Env::Default(), PB_MODEL_PATH,
                                            &graphdef);  //从pb文件中读取模型
  if (!status_load.ok()) {
  ...
  if (!status_create.ok()) {
    cout << "ERROR: Creating graph in session failed..."
 ...
 * @note
 * 预处理部分占用程序大部分效率，并且影响之后处理结果，应当作为主要优化方向
 */
void RM_ArmorFitted::imageProcessing(Mat frame, int my_color) {
  ...
  if (is_last_data_catch && !is_Lost_target) {
    roi.updateROI(src_img, is_Lost_target, lose_target_cnt, last_armor,
                  armor_depth);
    roi_img = roi.ROI_img;
  } else if (!is_Lost_target && !is_last_data_catch &&
             lose_target_cnt <= LOSE_CNT_MAX) {
    roi.updateROI(src_img, is_Lost_target, lose_target_cnt, last_armor,
                  armor_depth);
    roi_img = roi.ROI_img;
    lose_target_cnt += 1;
  ...
  switch (my_color) {
    case RED: {
      ...
      threshold(bin_img_color, bin_img_color, BLUE_ARMOR_COLOR_TH, 255,
                THRESH_BINARY);
#endif
    ...
    case BLUE: {
      ...
      threshold(bin_img_color, bin_img_color, RED_ARMOR_COLOR_TH, 255,
                THRESH_BINARY);
#endif
    ...
    default: {
      ...
      threshold(bin_img_color_1, bin_img_color_1, COLOR_TH_BLUE, 255,
                THRESH_BINARY);
      threshold(bin_img_color_2, bin_img_color_2, COLOR_TH_RED, 255,
                THRESH_BINARY);
      imshow("trackbar", trackbar_img);
#else
      int th = int((BLUE_ARMOR_GRAY_TH + RED_ARMOR_GRAY_TH) * 0.5);
      threshold(gray_img, bin_img_gray, th, 255, THRESH_BINARY);
      threshold(bin_img_color_1, bin_img_color_1, BLUE_ARMOR_COLOR_TH, 255,
                THRESH_BINARY);
      threshold(bin_img_color_2, bin_img_color_2, RED_ARMOR_COLOR_TH, 255,
                THRESH_BINARY);
#endif
  ...
  Mat element = getStructuringElement(
      MORPH_ELLIPSE, cv::Size(3, 7));  // MORPH_ELLIPSE,cv::Size(3,7)
...
void RM_ArmorFitted::armorFitted() {
  ...
  for (size_t j = 0; j < contours.size(); ++j) {
    ...
    if ((w_h_ratio < 0.4f) /*高宽比,角度筛选形状符合要求的灯条*/
      ...
      putText(dst_img, to_string(R_rect.size.height), put_h, FONT_HERSHEY_PLAIN,
              1, Scalar(100, 0, 100), 1, 8, false);
      Point put_w =
          Point(R_rect.center.x + roi.tl.x, R_rect.center.y + roi.tl.y + 20);
      putText(dst_img, to_string(R_rect.size.width), put_w, FONT_HERSHEY_PLAIN,
              1, Scalar(100, 200, 0), 1, 8, false);
#endif
    }
  }  //筛选灯条循环结束
  ...
  for (size_t i = 0; i < candidate_lights.size(); ++i) {
    for (size_t j = i + 1; j < candidate_lights.size(); ++j) {
      //区别出左右灯条
      if (candidate_lights[i].rect.center.x <
          candidate_lights[j].rect.center.x) {
      ...
      //灯条高度差
      float light_y_diff =
          fabs(left_light.rect.center.y - right_light.rect.center.y);
      bool is_height_diff_catch = (light_y_diff < (h1 + h2) * 0.5f);

      //灯条间的距离
      float light_distance =
          centerDistance(left_light.rect.center, right_light.rect.center);
      ...
      if (is_height_diff_catch && is_light_angle_catch &&
...
// MIN(armor_rect.size.width,armor_rect.size.height)/MIN(armor_rect.size.width,armor_rect.size.height);
// 数字识别
#if IS_NUMBER_PREDICT_OPEN == 1
...
#if SHOW_DEBUG_INFORMATION == 1
    putText(dst_img, to_string(armor_aspect_ratio), armor_rect.center,
            FONT_HERSHEY_PLAIN, 2, Scalar(150, 200, 0), 1, 8,
            false);  // 装甲宽高比
#endif
    ...
    for (int j = 0; j < 4; j++) {
      line(dst_img, vtx[j], vtx[(j + 1) % 4], Scalar(150, 200, 0), 2, 8,
           0);  //装甲板rect
    }
#endif
  ...
  putText(dst_img, to_string(light_distance_val), p, FONT_HERSHEY_PLAIN, 2,
          Scalar(255, 0, 255), 1, 8, false);  //紫色 灯条距离
  p = Point(armor_rect.center.x, armor_rect.center.y + 40);
  putText(dst_img, to_string(max_dis_th), p, FONT_HERSHEY_PLAIN, 2,
          Scalar(255, 255, 0), 1, 8, false);  //蓝色 最大高的n倍做上限
  p = Point(armor_rect.center.x, armor_rect.center.y + 60);
  putText(dst_img, to_string(light_dis_equ_maxh), p, FONT_HERSHEY_PLAIN, 2,
          Scalar(0, 100, 255), 1, 8, false);  //橙色 灯条距离与最大高之比
  p = Point(armor_rect.center.x, armor_rect.center.y + 80);
  putText(dst_img, to_string(w1_equ_w2), p, FONT_HERSHEY_PLAIN, 2,
          Scalar(0, 255, 255), 1, 8, false);  //黄色 灯条宽之比
  p = Point(armor_rect.center.x, armor_rect.center.y + 100);
  putText(dst_img, to_string(armor.depth), p, FONT_HERSHEY_PLAIN, 2,
          Scalar(194, 158, 241), 1, 8, false);  //粉色 深度距离信息
#endif
  ...
  // float focal =
  // pinhole_test.getfocalLength(armor_width,distance_f,true_width);
  // pinhole_test.getDistance(armor_width,focal,true_width);

  Point p = Point(100, 100);
  putText(src_img, "current distance:   " + to_string(distance / 1000), p,
          FONT_HERSHEY_PLAIN, 2, Scalar(255, 255, 255), 1, 8, false);
// p = Point(100,200);
// putText(src_img,"armor depth distance:   " +
// to_string(armor.depth/1000),p,FONT_HERSHEY_PLAIN,2,Scalar(255, 255,
// 255),1,8,false);

//发送串口数据
#if IS_SERIAL_OPEN == 1
  SerialPort::RMserialWrite(
      _yaw_data, yaw_data, _pitch_data, pitch_data, distance,
      is_last_data_catch,
      shooting);  // SerialPort::RMserialWrite(_yaw_data, abs(yaw_data),
                  // _pitch_data, abs(pitch_data), armor.depth,
                  // is_last_data_catch, shooting);
#endif
 ...
 * @return Mat
 */
Mat RM_ArmorFitted::CVMat_to_Tensor(Mat src, Tensor *output_tensor,
...
int RM_ArmorFitted::getLabel(Mat roi_img) {
  ...
  Status status_run = session->Run({{input_tensor_name, resizer_tensor}},
                                   {output_node}, {}, &outputs);
  if (!status_run.ok()) {
  ...
  auto tmap =
      t.tensor<float, 2>();  // Tensor Shape:[batch_size, target_class_num]
