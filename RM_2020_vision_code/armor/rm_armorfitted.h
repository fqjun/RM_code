...
//     上一次检测到装甲的旋转矩形 Point tl = Point(0,0); // 截图区域的左上角的点
//     int width = 0;                        // 截图区域的宽度
...
//      */
//     void updateROI(Mat &src,bool is_Lost_target,short int cnt,RotatedRect
//     last_roi_rect, double dis){
//         Point _tl;
//         int W,H;
//         if(!is_Lost_target){
...
//             }
//             /*--- ROI区域安全处理 ---*/
//             _tl = Point2f(last_roi_rect.center.x -
//             W*0.5f,last_roi_rect.center.y - H*0.5f); if (_tl.x < 0){
...
class ArmorROI {
 public:
   ...
   */
  void updateROI(Mat &src, bool is_Lost_target, short int cnt,
                 RotatedRect last_roi_rect, double dis) {
    Point _tl;
    int W, H;
    if (!is_Lost_target) {
      if (cnt <= LOSE_CNT_MAX * 0.5) {
      ...
      /*--- ROI区域安全处理 ---*/
      _tl = Point2f(last_roi_rect.center.x - W * 0.5f,
                    last_roi_rect.center.y - H * 0.5f);
      if (_tl.x < 0) {
        _tl.x = 0;  //如果左上角点x超出尺寸，x =  0
      }
      if (_tl.y < 0) {
  ...
  void setNumROI(Mat &src, RotatedRect &_rect) {
    ...
    RotatedRect rect =
        RotatedRect(_rect.center, cv::Size(roi_w, roi_h), _rect.angle);
    Point2f verices[4];
    rect.points(verices);
    for (int j = 0; j < 4; j++) {
      line(src, verices[j], verices[(j + 1) % 4], Scalar(150, 50, 100), 2, 8,
           0);
    }
    Point2f verdst[4];
    ...
    warpPerspective(src, roi_img, warpMatrix, roi_img.size(), INTER_LINEAR,
                    BORDER_CONSTANT);
#if IS_NUMBER_PREDICT_OPEN == 1
...
//         rect = _rect;
//         WH_ratio =
//         MIN(rect.size.width,rect.size.height)/MAX(rect.size.width,rect.size.height);
...
//     void inputParam(NiceLight &light_left, NiceLight &light_right,
//     RM_SolveAngle &angle_solve){
//         rect = fit_Rrect(light_left.rect, light_right.rect);
//         armor_width = centerDistance(light_left.rect.center,
//         light_right.rect.center); dist_to_center =
//         centerDistance((light_left.rect.center +
//         light_right.rect.center)*0.5,
//                                         IMG_CENTER);
//         float _w = float(sqrt(pow(fabs(light_left.depth - light_right.depth),
//         2) + pow(armor_width, 2)));
//         //RotatedRect _rect =
//         RotatedRect(rect.center,Size(_w,rect.size.height),rect.angle); float
//         temp_w = rect.size.width; rect.size.width = _w;
//         angle_solve.run_SolvePnp(rect, SMALL_ARMOR_SIZE_W,
//         SMALL_ARMOR_SIZE_H); rect.size.width = temp_w; depth =
//         int(angle_solve.dist); #if SERIAL_COMMUNICATION_PLAN == 0
//         /* 二维＋深度 */
...
class RM_ArmorFitted : public ArmorROI,
                       public NiceLight,
                       public CandidateArmor {
 public:
  RM_ArmorFitted();
  ~RM_ArmorFitted();
 ...
 private:
  ...
  int COLOR_TH_RED = RED_ARMOR_COLOR_TH;
#endif
...
#if IS_NUMBER_PREDICT_OPEN == 1
 private:
  Mat CVMat_to_Tensor(Mat src, Tensor *output_tensor, int input_rows,
                      int input_cols);
